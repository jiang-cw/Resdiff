...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        b, n, H, W, D = x.shape
        x = x.view(b, n, -1).permute(0, 2, 1)  # 将 x 重塑为 [batch, H*W*D, n]
        h = self.heads
        queries = self.query(x)
        keys = self.key(y)
        values = self.value(y)
        queries = queries.view(b, H*W*D, h, -1).transpose(1, 2)  # 修改 queries 的形状
        keys = keys.view(b, 1, h, -1).transpose(1, 2)  # 修改 keys 的形状
        values = values.view(b, 1, h, -1).transpose(1, 2)  # 修改 values 的形状
        dots = torch.einsum('bhij,bhij->bhij', queries, keys) * self.scale  # 修改 einsum 中的公式
...
class Diffusion:
    def __init__(self, noise_steps=15, min_noise_level=0.04, etas_end=0.999, kappa= 0.1, power=0.3):
