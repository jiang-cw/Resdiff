...
def Pix2Pix():
    ...
    gen = TransUNet().to(config.device)
    ...
    for epoch in range(config.epochs):
        ...
        lossfile.close()
        if epoch % 5 == 0:
            indicatorfile = open("result/"+str(config.exp)+"validation.csv", 'a+',newline = '')
            writer = csv.writer(indicatorfile)
            if epoch == 0:
                writer.writerow(["Epoch","MAE","PSNR","SSIM"])
            dataset = OneDataset(data_path = config.test, target_path = config.test_target, name = "validation")
            loader = DataLoader(dataset,batch_size=config.batch_size,shuffle=True,num_workers=config.numworker,pin_memory=True,drop_last=True)
            loop = tqdm(loader, leave=True)
            length = dataset.__len__()
            ...
            for idx, (t1, PET, data_name) in enumerate(loop):
                ...
                with torch.no_grad():
                    ...
                    fake_PET=fake_PET.astype(np.float64)

                    True_PET=PET.detach().cpu().numpy()
                    True_PET=np.squeeze(True_PET)
                    True_PET=True_PET.astype(np.float64)
            ...
            average_epoch = psnr_0_target + ssim_0_target*10
